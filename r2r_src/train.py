        ...
        for env_name, (env, evaluator) in val_envs.items():
            ...
            if bleu_score > best_bleu[env_name]:
                ...
                if env_name == 'val_unseen':
                    print()
                    print("EVALERR: {}%".format(best_bleu[env_name]))
                    print()
...
def valid(train_env, tok, val_envs={}):
    agent = Seq2SeqAgent(train_env, "", tok, args.maxAction,encoder_type=args.encoderType)
...
def train_val_augment():
    ...
    vocab = read_vocab(TRAIN_VOCAB)
    tok = Tokenizer(vocab=vocab, encoding_length=args.maxInput)
    stok = Tokenizer(vocab=vocab, encoding_length=args.maxInput)
    if args.encoderType in ['DicEncoder','CEncoder','Dic']:
        tok = BTokenizer(encoding_length=args.maxInput)
    ...
    # Start training
    train(train_env, tok, args.iters, val_envs=val_envs, aug_env=aug_env,stok=stok)

def train_all_augment():
    """
    Train the listener with the augmented data
    """
    setup()
    ...
    # Create the training environment
    train_env = R2RBatch(feat_dict, batch_size=args.batchSize,
                         splits=['train','val_seen','val_unseen'], tokenizer=tok)
    aug_env   = R2RBatch(feat_dict, batch_size=args.batchSize,
                         splits=[aug_path], tokenizer=tok, name='aug')
