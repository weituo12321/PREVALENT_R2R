...
from pretrain_class import BertAddPreTrain,BertAddPaPreTrain
...
class BERTTrainer:
    ...
    def __init__(self, model, vocab_size,
        ...
        if self.model.__class__.__name__ in ['DataParallel', 'DataParallelModel']:
        ...
        if with_cuda and torch.cuda.device_count() > 1:
            print("Using %d GPUS for BERT" % torch.cuda.device_count())
            #self.model = nn.DataParallel(self.model, device_ids=range(torch.cuda.device_count()))
            self.criterion = DataParallelCriterion(self.criterion, device_ids=range(torch.cuda.device_count()))
    ...
    def iteration(self, epoch, data_loader, train=True):
        ...
        for i, data in data_iter:
            ...
            if self.include_vision:
                ...
                output = self.model.forward(sorted_tensor.cuda(), mask.cuda(),seq_lengths.cuda(),f_t_all.cuda())
                ...
                output = self.model.forward(sorted_tensor.cuda(), mask.cuda(),seq_lengths.cuda(),None)
                length_output = len(output)
                print("You got %d outputs" % (length_output))
                next_sent_output, mask_lm_output = zip(*output)
...
def train():
    ...
    import glob
    jfiles = glob.glob("./test_traj" + "/*.json")
    ...
    model = BertAddPaPreTrain(FEATURE_ALL_SIZE, enc_hidden_size, hidden_size, dropout_ratio, bidirectional, transformer_update, bert_n_layers, reverse_input, top_lstm, vl_layers,bert_type, len(tok))
