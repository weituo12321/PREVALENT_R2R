...
from vilmodel import BertModel,BertImgModel,BertLayer,BertLayerNorm,BertPooler,BertAddModel,DicModel
...
class BertEncoder(nn.Module):
    ...
    def forward(self, inputs, mask, lengths):
        ...
        output = self.bert(inputs[:, :seq_max_len], token_type_ids=None,
                                          attention_mask=att_mask.to(device))
...
class VicEncoder(nn.Module):
    ...
    def __init__(self, vision_size, hidden_size, dec_hidden_size, dropout_ratio, bidirectional, update, bert_n_layers, reverse_input, top_lstm, vl_layers, pretrain = True, bert_type="small"):
    ...
    def forward(self, inputs, mask, lengths,f_t_all=None):
        ...
        if f_t_all is not None:
            ...
            img_seq_mask = torch.ones(batch_size, img_seq_len, dtype=att_mask.dtype).to(att_mask.device)
            mask = torch.cat((~img_seq_mask, mask),dim=1)
            visn_att_mask = img_seq_mask
            lang_att_mask = att_mask

            lang_extended_attention_mask = lang_att_mask.unsqueeze(1).unsqueeze(2)
            lang_extended_attention_mask = lang_extended_attention_mask.to(dtype=next(self.parameters()).dtype) # fp16 compatibility
            lang_extended_attention_mask = (1.0 - lang_extended_attention_mask) * -10000.0

            visn_extended_attention_mask = visn_att_mask.unsqueeze(1).unsqueeze(2)

            visn_extended_attention_mask = visn_extended_attention_mask.to(dtype=next(self.parameters()).dtype) # fp16 compatibility
            visn_extended_attention_mask = (1.0 - visn_extended_attention_mask) * -10000.0


            lang_ctx, visn_ctx = self.addlayer(text_embeds, lang_extended_attention_mask, img_embedding_output, visn_extended_attention_mask)
            embeds = lang_ctx
        else:
            embeds = text_embeds
