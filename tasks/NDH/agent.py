...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, encoder, decoder, episode_len=20, path_type='planner_path',args=None):
        ...
        if self.decoder.__class__.__name__ == '':
    ...
    def vl_rollout(self):
        ...
        for t in range(self.episode_len):
            ...
            #if self.encoder.__class__.__name__ in ['BertImgEncoder','DicEncoder','VicEncoder']:
            f_t_all = f_t_all[:,::2,:]   # segment sample
    ...
    def owntest(self):
        ...
        looped = False
        while True:
            if self.encoder.__class__.__name__ == 'BertEncoder':
    ...
    def train(self, encoder_optimizer, decoder_optimizer, n_iters, feedback='teacher'):
        ...
        for iter in range(1, n_iters + 1):
            ...
            if self.encoder.__class__.__name__ == 'EncoderLSTM':
                self.rollout()
            elif self.encoder.__class__.__name__ == 'BertEncoder':
                self.bert_rollout()
            elif self.encoder.__class__.__name__ in ['DicEncoder']:
                self.vl_rollout()
            else:
                raise NotImplementedError
            self.loss.backward()
...
class OSeq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, encoder, decoder, episode_len=20, path_type='planner_path'):
        super(OSeq2SeqAgent, self).__init__(env, results_path)
        self.encoder = encoder
        self.decoder = decoder
        self.episode_len = episode_len
        self.losses = []
